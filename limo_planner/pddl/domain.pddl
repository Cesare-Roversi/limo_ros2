(define (domain robot_domain)
  ...
  ; check_distance ESCLUSA dal lock doing_nothing: non ha un parametro robot,
  ; quindi resta libera di eseguire quando vuole (nessuna precondizione).
  ...
  (:durative-action retract_arm
    :parameters (?r - robot)
    :duration (= ?duration 1)
    :condition (at start (doing_nothing ?r))
    :effect (and
